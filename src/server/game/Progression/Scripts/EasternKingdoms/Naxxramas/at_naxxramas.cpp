...
public:
    ...
    bool OnTrigger(Player* player, AreaTrigger const* trigger)
    {
        ...
        if ((player->GetQuestStatus(QUEST_THE_DREAD_CITADEL_NAXXRAMAS) != QUEST_STATUS_REWARDED || player->GetLevel() < 51 || !player->GetGroup() || (player->GetGroup() && !player->GetGroup()->isRaidGroup())) && !player->IsGameMaster())
        {
