...
SELECT `id`, `map_id`, `difficulty`, `min_level`, `max_level`, `min_avg_item_level`, `comment`, 19 AS `MinPatch`, 21 AS `MaxPatch` FROM `base_world`.`dungeon_access_template` WHERE `map_id` IN (
    249 -- Onyxia's Lair
) UNION SELECT `id`, `map_id`, `difficulty`, `min_level`, `max_level`, `min_avg_item_level`, `comment`, 17 AS `MinPatch`, 21 AS `MaxPatch` FROM `base_world`.`dungeon_access_template` WHERE `map_id` IN (
    533 -- Naxxramas
